...
describe("identificarMoeda", () => {
  it("identifica o Real para datas a partir de 01/07/1994", () => {
    expect(identificarMoeda(new Date(1994, 6, 1)).codigo).toBe("real")
    expect(identificarMoeda(new Date(2026, 0, 1)).codigo).toBe("real")
  })

  it("identifica o Cruzeiro Real entre 01/08/1993 e 30/06/1994", () => {
    expect(identificarMoeda(new Date(1993, 7, 1)).codigo).toBe("cruzeiro_real")
    expect(identificarMoeda(new Date(1994, 2, 1)).codigo).toBe("cruzeiro_real")
    expect(identificarMoeda(new Date(1994, 5, 30)).codigo).toBe("cruzeiro_real")
  })

  it("identifica o Cruzeiro (1990-1993) entre 16/03/1990 e 31/07/1993", () => {
    expect(identificarMoeda(new Date(1990, 2, 16)).codigo).toBe("cruzeiro_1990")
    expect(identificarMoeda(new Date(1991, 4, 15)).codigo).toBe("cruzeiro_1990")
    expect(identificarMoeda(new Date(1993, 6, 31)).codigo).toBe("cruzeiro_1990")
  })

  it("identifica o Cruzado Novo entre 16/01/1989 e 15/03/1990", () => {
    expect(identificarMoeda(new Date(1989, 0, 16)).codigo).toBe("cruzado_novo")
    expect(identificarMoeda(new Date(1990, 2, 15)).codigo).toBe("cruzado_novo")
  })
...
describe("converterUFESP", () => {
  it("identifica a moeda do próprio registro pela data de vigência", () => {
    ...
    const resultado = converterUFESP(1996.31, new Date(1991, 4, 15))
    expect(resultado.moeda.codigo).toBe("cruzeiro_1990")
...
describe("resolverMoeda", () => {
  it('com "auto", detecta pela data de referência', () => {
    ...
    expect(regime.codigo).toBe("cruzeiro_1990")
  })
  ...
  it("com um código explícito, ignora a data e usa o código escolhido", () => {
    ...
    expect(regime.codigo).toBe("cruzeiro_real")
  })
})
